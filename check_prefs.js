const https = require("https");

const url =
  "https://syd.cloud.appwrite.io/v1/databases/694696b60033338761f5/collections/preferences/attributes";
const options = {
  headers: {
    "X-Appwrite-Project": "690621dc00241683b91a",
    "X-Appwrite-Key":
      "standard_",
  },
};

https
  .get(url, options, (res) => {
    let data = "";
    res.on("data", (chunk) => {
      data += chunk;
    });
    res.on("end", () => {
      if (res.statusCode >= 200 && res.statusCode < 300) {
        const response = JSON.parse(data);
        console.log(
          "Preferences Collection - Total Attributes:",
          response.total
        );
        response.attributes.forEach((attr) => {
          console.log(
            `  ${attr.key} -> type:${attr.type}, required:${
              attr.required
            }, array:${attr.array || false}`
          );
        });
      } else {
        console.error("Error:", res.statusCode, data);
      }
    });
  })
  .on("error", (err) => {
    console.error("Request error:", err);
  });
